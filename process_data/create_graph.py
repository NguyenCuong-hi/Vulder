...
def filter_nodes_with_code_property(nodes):
    ...
    for node_index, node in enumerate(nodes):
        key = str(list(node.keys())).strip('[]').split(',')
        value = str(list(node.values())).strip('[]').split(',')
        if "CODE:string" in key:
            id = value.pop(0)
...
def create_adjacency_list(line_numbers, node_id_to_line_numbers, edges, data_dependency_only=False):
    adjacency_list = {}
    key = ',:START_ID,:END_ID,:TYPE'
    for ln in set(line_numbers):
        adjacency_list[ln] = [set(), set()]
    for edge in edges:
        edge_type = edge[key].split(',')[-1]
        if True:  # edge_type in ['IS_AST_PARENT', 'FLOWS_TO']:
            start_node_id = edge[key].split(',')[-3]
            end_node_id = edge[key].split(',')[-2]
            node_id_to_line_numbers.keys()
...
def create_graph_from_nodes_and_edges(nodes_ids_to_nodes, edges, allowed_edge_types):
    ...
    for edge in edges:
        ...
        if edge_type in allowed_edge_types.keys():
            start_node_id = edge[key].split(',')[-3]
            end_node_id = edge[key].split(',')[-2]
            get_value_key_id = str(nodes_ids_to_nodes.keys()).strip("''\"").replace('dict_keys(["','').replace('"', '').replace(',"', ',').replace("'", '').replace('])','').split(',')
            list_id = [element.strip(" ") for element in get_value_key_id]
            if start_node_id in list_id and end_node_id in list_id:
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--code', help='Name of code file', default='D:/NCKH/ReVeal/data/raw_code/source/0_0.c')
    ...
    directory_edges = 'D:\\NCKH\\vuldet\\Vulder\\data\\data_csvout_10\\edges.csv'
    directory_nodes = 'D:\\NCKH\\vuldet\\Vulder\\data\\data_csvout_10\\nodes.csv'

    # file_name = args.code
    # slice_ln = int(args.line)
    # code_file_path = os.path.join(directory, file_name)
    code_file_path = "D:\\NCKH\\vuldet\\Vulder\\data\\source\\10_1.c"
    data_output = "D:\\NCKH\\vuldet\\Vulder\\data\\output\\graph"
    # nodes_path = os.path.join( directory, file_name, 'nodes.csv')
    # edges_path = os.path.join( directory, file_name, 'edges.csv')
    nodes = read_csv(directory_nodes)
    edges = read_csv(directory_edges)
    ...
    create_visual_graph(code, adjacency_list, data_output)
